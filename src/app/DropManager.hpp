...
public:
  ...
  HRESULT DragEnter(IDataObject *pDataObj, DWORD grfKeyState, POINTL pt,
                    DWORD *pdwEffect) {
    // TODO: check whether we can handle this type of object at all and set
    // *pdwEffect &= DROPEFFECT_NONE if not;
  ...
  // occurs when we drag the mouse over our applications view whilst carrying
  // files (post Enter, pre Leave)
  ...
  HRESULT Drop(IDataObject *pDataObj, DWORD grfKeyState, POINTL pt,
               DWORD *pdwEffect) {
    // grfKeyState contains flags for control, alt, shift etc
    ...
    if (SUCCEEDED(pDataObj->GetData(&fmte, &stgm))) {
      ...
      // we can drag more than one file at the same time, so we have to loop
      // here
