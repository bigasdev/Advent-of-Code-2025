...
void Timer::update() {
  ...
  elapsed_time = (double)(new_time - last_time_stamp) /
...
double Timer::get_tmod() { return dt * tmod_multiplier; }

// delta time, use it for everything else and stuff that is at update() or
// post_update()
double Timer::get_dt() { return current_dt * tmod_multiplier; }

double Timer::get_accumulator() { return accumulator; }

float Timer::get_fps() { return 1 / current_dt; }

int Timer::get_frame_count() { return frame_count; }
// skip a frame
void Timer::skip() { last_time_stamp = SDL_GetPerformanceCounter(); }

// hooked to the main.cpp while loop, this ensures the fixed delta time loop
// occurs
void Timer::fixed_t() {
  dt = fixed_dt;
...
void Timer::reset_slow_mo() { tmod_multiplier = 1.0; }
